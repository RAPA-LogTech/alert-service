...
async def get_slack_status() -> dict:
    ...
    return {
        ...
        "installedAt": installation_config.get("updated_at")
...
async def disconnect_slack() -> dict:
    ...
    if not success:
        raise HTTPException(status_code=500, detail="Slack 연동 해제 중 오류가 발생했습니다.")
...
async def update_slack_config(payload: UpdateChannelPayload) -> dict:
    ...
    if not success:
        raise HTTPException(status_code=500, detail="채널 변경 중 오류가 발생했습니다.")
